...
**ワークフローは 1 ステップずつ順に実行します。**依存のないステップ同士も同様なので、全体の所要時間は各ステップの合計です。局所性の重みを 0 にしても独立した枝は分散しません — 実測して確認しました。逐次実行では奪い合いが起きないので、分散させる材料がそもそも無いためです。どちらも設計上の選択ではなく制約です。[`examples/11-workflow`](examples/11-workflow)。
