...
正直な限界: **コントローラを再起動した場合は何も再開しません**。記録は残りますが、
どのデータがどこにあるかの目録はメモリ上にあり、再起動で消えます。したがって全ステップが
走り直します（これも実測です）。使えるのは、メッシュは生きたままワークフローだけが途中で
失敗した、という普通のケースです。再起動でも再開させるには、エージェント側が保持データを
再申告する必要があり、それはプロトコルの変更なので、ここではやりません。
