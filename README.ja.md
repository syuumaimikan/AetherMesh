...
- **Recent tasks** — この画面からではなく、**メッシュ全体で**終わったタスク。上の `wasm` の行は別のターミナルで TypeScript から投げたものです。「さっき投げたやつ、動いた？どこで？」に、投げた端末に戻らずに答えられます。バイナリ出力が点になっているのは、タスクの出力が任意のバイト列で、端末はその中のエスケープを喜んで解釈してしまうからです。表示は出力の先頭部分だけで、本当のサイズは投げた側が受け取っています
...
いま残っているもの。上から順に、価値が高い順です。

- [ ] **実機・実ネットワークで動かすこと。** このリポジトリで最も弱い主張です。
      数字はすべてループバックのもので、この 1 台の上でどれだけ作業しても変わりません
- [ ] クラウドアダプタを実アカウントで動かし、そこで分かったことを直す
- [ ] サンドボックスと認証経路を、書いていない人がレビューする
- [ ] コントローラの複数化。いまは 1 プロセスで、落とすとエージェントが再申告するまで
      データの所在が失われます
- [ ] QUIC。フレーミング層はすでに transport 非依存ですが、QUIC が効くかどうかは
      実ネットワークの話で、それをまだ誰も測っていません
- [ ] コストをスコアの項に入れた広域スケジューリング

1 番目が 5 番目の前提です。ネットワークを測ったことがないまま、ネットワークを速くする
transport を足すのは、誰も聞いていない質問への丁寧な答えを作ることです。
